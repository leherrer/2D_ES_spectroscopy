...
from util_HEOM import Hsys, Baths, evolve_HEOM, N_ADOS 
from util_HEOM import  rho0, options , baths , terminator, N_ADO , mu_p0, mu_m0, NC, Nk
from util_2D_eigen import R_signal_para , Fourier_Transfor, Resp_para_HEOM , normalize_intensities ,R_signal_para_pathway

print(rho0.shape)
gam =((1/gamma*hbar))
print(gam)

print(Resp_para_HEOM(0,0,0,0,0,0))
...
np.save("Rsignal_rp_HEOM_J-%0.1f_l-%0.1f.npy"%(J, lam), Rsignal_rp)
np.save("Rsignal_nr_HEOM_J-%0.1f_l-%0.1f.npy"%(J, lam), Rsignal_nr)
...
for t2, spectrum in zip(t2s, spectra):
                with open('2d_t2-%0.1f_HEOM_dt-%0.0f_tf-%0.0f_L-%d_K-%d_g-%0.1f_l-%0.1f.dat'%(t2,dt,t_final,NC,Nk, gam, lam), 'w') as f:
